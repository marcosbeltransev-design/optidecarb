...
from industrial_energy_lab.optimization.sensitivity import SENSITIVITY_FAMILIES, battery_break_even_from_sensitivity
...
def _economics(st) -> None:
    ...
    c1, c2, c3 = st.columns(3)
    pv_capex = (r.pv_capacity_kw or 0.0) * float(st.session_state.params["pv_capex_eur_per_kw"])
    battery_capex = (r.battery_energy_capacity_kwh or 0.0) * float(st.session_state.params["battery_energy_capex_eur_per_kwh"]) + (r.battery_power_capacity_kw or 0.0) * float(st.session_state.params["battery_power_capex_eur_per_kw"])
    with c1: _metric(st, "pv_capex", format_eur(pv_capex))
    with c2: _metric(st, "battery_capex", format_eur(battery_capex))
    with c3: _metric(st, "initial_capex", format_eur(r.initial_capex_eur))
...
def _sensitivity(st) -> None:
    ...
    if len(optimal) >= 2:
        ...
        pv_delta = (last["pv_capacity_kw"] or 0) - (first["pv_capacity_kw"] or 0)
        if abs(pv_delta) > 1e-6:
            direction = "increases" if pv_delta > 0 else "decreases"
            st.info(f"Across the solved range, optimal PV capacity {direction} from {first['pv_capacity_kw']/1000:.2f} MW to {last['pv_capacity_kw']/1000:.2f} MW. This statement is derived from the solved endpoints, not generated by an AI model.")
    if variable == "battery_capex_multiplier":
        threshold = battery_break_even_from_sensitivity(frame)
        if threshold is not None:
            tested = sorted(float(v) for v in frame["input_value"].unique())
            lower = max((v for v in tested if v < threshold), default=None)
            if lower is not None:
                st.info(
                    f"Storage changes from positive capacity to zero between the tested battery-CAPEX "
                    f"multipliers {lower:.2f}× and {threshold:.2f}×. This is an observed bracket, not a "
                    "more precise break-even claim."
                )
