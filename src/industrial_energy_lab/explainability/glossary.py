...
GLOSSARY = {
    ...
    "baseline": _g("baseline","Baseline","Baseline Scenario","The reference case before adding the new investment.","In IEL the baseline is grid-only electricity supply using the same load and price series.","—","Savings and CO2 reductions are measured relative to baseline.",( "objective_function","abatement_cost"),"A comparison needs a consistent reference.","Baseline is not necessarily business-as-usual forever; it is the defined reference model."),
    "lp": _g("lp","LP","Linear Programming","A mathematical method that chooses the best values for variables while obeying linear rules.","Optimization with a linear objective and linear equality/inequality constraints.","—","IEL chooses PV MW, battery MWh/MW and hourly dispatch to minimize annualized cost.",( "decision_variable","objective_function","constraint"),"LP makes the sizing problem transparent and efficiently solvable over 8,760 hours.","The solver does not 'predict' the future; it optimizes the stated model."),
    ...
    "objective_function": _g("objective_function","Objective","Objective Function","The number the optimizer tries to make as small as possible.","IEL minimizes annualized PV + battery cost + grid purchases - export revenue.","€/year","Two feasible systems are compared by their objective cost.",( "lp","constraint","baseline"),"It mathematically defines what 'economic optimum' means.","The objective does not include every real-world consideration."),
